...
src/js/codegen/unit-config.js: Engine sinh Unit Config JSON (v3). Tính địa chỉ MR/DM cho cylinder, sensor, solenoid. Tải và biên dịch Handlebars templates từ thư mục `src/templates/` để sinh mã IL; tự động fallback sang JS generator nếu Handlebars không khả dụng.
...
- Thứ tự tải script (quan trọng): Handlebars CDN → core/utils.js → core/graph-utils.js → core/store.js → core/constants.js → editor/* → codegen/*. Các biến global phải được khai báo trước khi dùng.
...
- Templating: Handlebars.js v4.7.9 (CDN). Dùng để sinh mã IL từ file `.hbs` trong src/templates/. Xem src/js/codegen/unit-config.js.
